...
```
kicaddy/
    models.py    # Library, Symbol, SymbolProperty, Footprint dataclasses + LibraryType enum
...
- **Migration**: `create_schema` issues `ALTER TABLE symbol ADD COLUMN footprint_id`
  wrapped in a try/except so existing databases are upgraded transparently.
